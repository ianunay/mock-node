...
if (argv.location) console.log(__dirname);else if (argv.import) {
  ...
  (function () {
    ...
    var logRequest = function logRequest(_req, _type, _method) {
      logger[_type][_method]('route: ' + _req.path + ', query strings: ' + JSON.stringify(_req.query) + ', request body: ' + JSON.stringify(_req.body) + ', ip: ' + _req.ip);
    };

    // Logs all requests which do not start with '/mocknode/'
    router.use('/', function (req, res, next) {
      if (/^(?!\/mocknode\/)/.test(req.originalUrl)) logRequest(req, 'accesslog', 'info');
      next();
    });
